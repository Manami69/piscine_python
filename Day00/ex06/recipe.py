...
def print_recipe(name):
    ...
    if verify == None:
        print("There is no {} recipe in the cookbook, please add one".format(name))
    else:
...
def add_recipe(name, ingredients, meal, prep_time):
    new_entry = {name : {'ingredients' : ingredients, 'meal' : meal, 'prep_time' : prep_time}}
...
def ask_for_recipe(request):
    recipe = str(input("Please enter the recipe's name to get its details:\n>>"))
    if request == 'add':
        print_recipe(recipe)
    if request == 'del':
        del_recipe(recipe)
...
def give_new():
    ...
    while True:
            next_i = input("Add an ingredient to your recipe (put 0 when you're done)\n>> ")
            if (next_i == '0'):
                break
